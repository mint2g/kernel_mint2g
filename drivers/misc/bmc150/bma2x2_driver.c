		...
		const char *buf, size_t count)
{
	...
	if(data){
		...
		printk(KERN_INFO "[BMC150] (%d,%d,%d)",cal_data[0],cal_data[1],cal_data[2]);
...
static struct regulator *VSENSOR_3_0_V;
#endif
...
static int __init BMA2X2_init(void)
{
        ...
        VSENSOR_3_0_V = regulator_get(NULL, REGU_NAME_SENSOR);
        if(IS_ERR(VSENSOR_3_0_V)){
                printk(KERN_ERR "[BMA2X2] can not get VSENSOR_3.0V\n");
        }

        ret = regulator_is_enabled(VSENSOR_3_0_V);
        printk(KERN_INFO "[BMA2X2] regulator_is_enabled : %d\n", ret);

         ret = regulator_set_voltage(VSENSOR_3_0_V, 3000000, 3000000);
        printk(KERN_INFO "[BMA2X2] regulator_set_voltage : %d\n", ret);

         ret = regulator_enable(VSENSOR_3_0_V);
        printk(KERN_INFO "[BMA2X2] regulator_enable : %d\n", ret);

        /*After Power Supply is supplied, about 1ms delay is required before issuing read/write commands */
        mdelay(10);
#endif

        printk(KERN_INFO "[BMA2X2] %s\n",__FUNCTION__);
