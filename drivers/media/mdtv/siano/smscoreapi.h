...
#define LOAD_FIRMWARE_FROM_FILE
...
struct smscore_device_t {
	...
	struct completion gpio_get_level_done, ir_init_done, max_tx_done;
#if APP_IN_DRIVER
	struct completion periodic_stat_done, tune_done, scan_done, scan_complete, start_service_done;
...
#else
#define sms_debug(fmt, arg...) \
	dprintk(KERN_INFO, DBG_ADV, fmt, ##arg)
#endif
