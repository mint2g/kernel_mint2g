...
static struct smsmdtv_version_t version = {2,2,2};
...
extern int smsspi_driver_init(void);
extern void smsspi_driver_exit(void);
...
int smscore_register_hotplug(hotplug_t hotplug)
{
	...
	notifyee = kmalloc(sizeof(struct smscore_device_notifyee_t),
		GFP_KERNEL);
    sms_info("notifyee 0x%x", (unsigned long)notifyee);
	if (notifyee) {
...
int smscore_register_device(struct smsdevice_params_t *params,
	...
	init_completion(&dev->max_tx_done);
#if APP_IN_DRIVER
	init_completion(&dev->periodic_stat_done);
	init_completion(&dev->tune_done);
...
#if 1 //original
	sms_info("JACKZ, dma_alloc_coherent\n");
	dev->common_buffer = dma_alloc_coherent(NULL, dev->common_buffer_size,
			&dev->common_buffer_phys, GFP_KERNEL | GFP_DMA);
	/* sms_info("dma_alloc: virt 0x%x, phys 0x%x, size %d",
                dev->common_buffer,
                dev->common_buffer_phys,
	            dev->common_buffer_size); */
#else
    dev->common_buffer = __get_free_pages(GFP_KERNEL | GFP_DMA, 
            get_order(dev->common_buffer_size));
//    dev->common_buffer  = kzalloc(dev->common_buffer_size, GFP_KERNEL | GFP_DMA);
    dev->common_buffer_phys = virt_to_phys(dev->common_buffer);
    sms_info("kzalloc: virt 0x%x, phys 0x%x, size %d",
                dev->common_buffer, 
                dev->common_buffer_phys,
                dev->common_buffer_size);
#endif
	...
	dev->board_id = SMS1XXX_BOARD_SIANO_MING;
...
#if 0//def REQUEST_FIRMWARE_SUPPORTED
	int mode;
...
static int smscore_load_firmware_family2(struct smscore_device_t *coredev,
	...
	if (!msg)
		return -ENOMEM;
    sms_info("mode is %d", coredev->mode);
...
#if 0
  {
    struct SmsMsgData_ST *maxTxMsg = (struct SmsMsgData_ST*)msg;

    sms_info("Enable 64KB transfer");
    SMS_INIT_MSG(msg, MSG_SMS_SET_MAX_TX_MSG_LEN_REQ,
            sizeof(struct SmsMsgHdr_ST) + 
            sizeof(u32) * 1);
    maxTxMsg->msgData[0] = 64*1024;

    smsendian_handle_tx_message((struct SmsMsgHdr_ST *)msg);
    rc = smscore_sendrequest_and_wait(coredev,
        maxTxMsg,
        maxTxMsg->xMsgHeader.msgLength,
        &coredev->max_tx_done);

    if(rc < 0)
    {
      sms_err("Enable 64KB transfer failed, rc %d", rc);
    }
  }
#endif
exit_fw_download:
...
void smscore_unregister_device(struct smscore_device_t *coredev)
{
	...
	//	free_pages(coredev->common_buffer, get_order(coredev->common_buffer_size));
...
{
	int rc = 0;
    	sms_info("device mode %d, mode supported %d", coredev->mode,coredev->modes_supported);
...
int smscore_set_device_mode(struct smscore_device_t *coredev, int mode)
{
	...
	if (coredev->device_flags & SMS_DEVICE_FAMILY2) {
        ...
        if (buffer)
        {
          ...
          if(rc<0)
          {
            printk("smscore_set_device_mode, failed in init dev\n");
          }

          printk("Start to get version\n");
          ...
          if(rc<0)
          {
            printk("smscore_set_device_mode, failed in get version\n");
          }

        printk("Set Periodic Stat before scan\n");
          ...
          if(rc<0)
          {
            printk("smscore_set_device_mode, failed in set periodic stat, rc=%d\n", rc);
          }

#if 0
        printk("Start to Tune\n");
  	    SMS_INIT_MSG(&msg->xMsgHeader, MSG_SMS_RF_TUNE_REQ,
  					sizeof(struct SmsMsgHdr_ST) + 2*sizeof(u32));
  	    msg->msgData[0] = 530000000;
        msg->msgData[1] = BW_8_MHZ;
          ...
          if(rc<0)
          {
            printk("smscore_set_device_mode, failed in tune, rc=%d\\n", rc);
          }
#else
...
#if 0
        msleep(1000);
        printk("Start Control Info\n");
  	    SMS_INIT_MSG(&msg->xMsgHeader, MSG_SMS_CMMB_START_CONTROL_INFO_REQ,
  					sizeof(struct SmsMsgHdr_ST) + 2*sizeof(u32));
        ...
        msg->msgData[1] = 0xFFFFFFFF;
  	    ...
  	    rc = smscore_sendrequest_and_wait(coredev, msg,
  					msg->xMsgHeader.msgLength,
  					&coredev->start_control_info_done);
        msleep(3000*10);
        printk("Start control info end\n");

        msleep(3000);
        printk("Stop Control Info\n");
  	    SMS_INIT_MSG(&msg->xMsgHeader, MSG_SMS_CMMB_STOP_CONTROL_INFO_REQ,
  					sizeof(struct SmsMsgHdr_ST));

  	    smsendian_handle_tx_message((struct SmsMsgHdr_ST *)msg);
  	    rc = smscore_sendrequest_and_wait(coredev, msg,
  					msg->xMsgHeader.msgLength,
  					&coredev->stop_control_info_done);
#endif
        msleep(5000);
        printk("Start service\n");
...
void smscore_onresponse(struct smscore_device_t *coredev,
	...
	if (rc < 0) {
		...
		case MSG_SMS_RF_TUNE_RES:
			printk("MSG_SMS_RF_TUNE_RES, length: 0x%x\n", msgLen);
			complete(&coredev->tune_done);
			break;
#else
		case MSG_SMS_RF_TUNE_RES:
    ...
    case MSG_SMS_GET_STATISTICS_EX_RES:
      {
		...
		case MSG_SMS_SET_MAX_TX_MSG_LEN_RES:
			sms_debug("MSG_SMS_SET_MAX_TX_MSG_LEN_RES");
			complete(&coredev->max_tx_done);
			break;
...
#if 1
		sms_devp=(struct sms_char_dev*)kmalloc(sizeof(struct sms_char_dev),GFP_KERNEL);
		sms_devp->dev_class=class_create(THIS_MODULE,"SMS_DTV");
		if(IS_ERR(sms_devp->dev_class)){
			printk(KERN_INFO"dev_class create error!\n");
			goto class_create_err;
		}
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,0),NULL,"mdtvctrl");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,1),NULL,"mdtv1");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,2),NULL,"mdtv2");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,3),NULL,"mdtv3");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,4),NULL,"mdtv4");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,5),NULL,"mdtv5");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,6),NULL,"mdtv6");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,7),NULL,"mdtv7");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,8),NULL,"mdtv8");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,9),NULL,"mdtv9");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,10),NULL,"mdtv10");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,11),NULL,"mdtv11");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,12),NULL,"mdtv12");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,13),NULL,"mdtv13");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,14),NULL,"mdtv14");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,15),NULL,"mdtv15");
		sms_devp->device1=device_create(sms_devp->dev_class,NULL,MKDEV(240,16),NULL,"mdtv16");
	class_create_err:
		kfree(sms_devp);
#endif
