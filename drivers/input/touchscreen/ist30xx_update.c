...
int ist30xx_isp_fw_recovery(struct i2c_client *client, const u8 *buf)
{
	int ret = 0;
	int i, j;
	u16 addr = EEPROM_BASE_ADDR;
	...
	DMSG("[ TSP ] *** F/W recovery (0x%04x~0x%04x) ***\n",
...
{
	return ist30xx_parse_param_ver(PARSE_FLAG_FW, ist30xx_fw);
}
...
int ist30xx_auto_fw_update(struct ist30xx_data *data)
{
	...
	if ((data->fw.ver > 0) && (data->fw.ver < 0xFFFFFFFF) &&
	    (data->param_ver > 0) && (data->param_ver < 0xFFFFFFFF)) {
...
int ist30xx_auto_bin_update(struct ist30xx_data *data)
{
	...
	if (!tsp_check) {
		retry = 1;
		while (retry--) {
			ret = ist30xx_isp_fw_recovery(data->client, ist30xx_fw);
			if (ret == 0) {
				ret = ist30xx_write_cmd(data->client, CMD_RUN_DEVICE, 0);
...
int ist30xx_force_bin_update(struct ist30xx_data *data)
{
	int ret = 0;
	int retry = IST30XX_FW_UPDATE_RETRY;
	u32 ist30xx_param_ver = 0;
...
/* sysfs: /sys/class/touch/firmware/recovery */
ssize_t ist30xx_recovery_store(struct device *dev, struct device_attribute *attr,
			       const char *buf, size_t size)
{
	...
	if (mode == 3) {
		...
		mutex_lock(&ist30xx_mutex);
		ist30xx_isp_fw_recovery(ts_data->client, fw);
		mutex_unlock(&ist30xx_mutex);
...
static DEVICE_ATTR(recovery, S_IRUGO | S_IWUSR | S_IWGRP, ist30xx_fw_show, ist30xx_recovery_store);
static DEVICE_ATTR(version, S_IRUGO | S_IWUSR | S_IWGRP, ist30xx_fw_version, NULL);
static DEVICE_ATTR(param, S_IRUGO | S_IWUSR | S_IWGRP, NULL, ist30xx_param_store);
...
int ist30xx_init_update_sysfs(void)
{
	...
	/* /sys/class/touch/firmwware/recovery */
	if (device_create_file(ist30xx_fw_dev, &dev_attr_recovery) < 0)
		pr_err("[ TSP ] Failed to create device file(%s)!\n",
		       dev_attr_recovery.attr.name);
