...
#define MAX_X	240
#define MAX_Y	320
#define TSP_INT IRQ_GPIO_INT

#define GPIO_TP_SDA	16
#define GPIO_TP_SCL	18
#define GPIO_TP_INT	17
...
static struct regulator *touch_regulator1 = NULL;
//add by brcm
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr,
			     char *buf);
static ssize_t firmware_ret_show(struct device *dev,
...
static ssize_t rawdata_show_silabs(struct device *dev,
				   struct device_attribute *attr, char *buf);
static ssize_t raw_enable_silabs(struct device *dev,
				 struct device_attribute *attr, char *buf);
static ssize_t raw_disable_silabs(struct device *dev,
				  struct device_attribute *attr, char *buf);
static ssize_t baseline_show_silabs(struct device *dev,
				    struct device_attribute *attr, char *buf1);
static ssize_t diff_show_silabs(struct device *dev,
				struct device_attribute *attr, char *buf);
static ssize_t fimware_show_versname(struct device *dev,
				     struct device_attribute *attr, char *buf);
static ssize_t rawdata_pass_fail_silabs(struct device *dev,
					struct device_attribute *attr,
					char *buf);
static ssize_t read_node(struct device *dev, struct device_attribute *attr,
			 char *buf);
...
void touch_ctrl_regulator(int on_off)
{
	if (on_off == TOUCH_ON) {
		...
		regulator_set_voltage(touch_regulator1, 3000000, 3000000);
		regulator_enable(touch_regulator);
		regulator_enable(touch_regulator1);
	} else {
		regulator_disable(touch_regulator);
		regulator_disable(touch_regulator1);
	}
}
...
int tsp_reset(void)
{
	...
	gpio_direction_output(GPIO_TP_SCL, 0);
	gpio_direction_output(GPIO_TP_SDA, 0);
	...
	gpio_direction_output(GPIO_TP_SDA, 1);
	gpio_direction_output(GPIO_TP_SCL, 1);

	gpio_direction_input(GPIO_TP_INT);
	gpio_direction_input(GPIO_TP_SDA);
	gpio_direction_input(GPIO_TP_SCL);
...
void TSP_forced_release_forkey(void)
{
	...
	for (i = 0; i < SILABS_MAX_TOUCH; i++) {
		...
		input_report_abs(ts_global->input_dev, ABS_MT_POSITION_X,
				 g_Mtouch_info[i].posX);
		input_report_abs(ts_global->input_dev, ABS_MT_POSITION_Y,
				 g_Mtouch_info[i].posY);
		input_report_abs(ts_global->input_dev, ABS_MT_TOUCH_MAJOR, 0);
		input_report_abs(ts_global->input_dev, ABS_MT_WIDTH_MAJOR,
	...
	for (key = 0; key < MAX_KEYS; key++) {
		...
		input_report_key(ts_global->input_dev,
				 touchkey_status[key].key_value,
				 touchkey_status[key].key_press);
	}
...
static irqreturn_t silabs_ts_work_func(int irq, void *dev_id)
{
	...
	for (i = 0; i < I2C_RETRY_CNT; i++) {
		...
		if (ret >= 0) {
			ret =
			    i2c_master_recv(ts->client, buf, TS_READ_REGS_LEN);
	...
	if (ret < 0) {
...
#ifdef __TOUCH_DEBUG__
		printk
		    ("[TSP] button_num : %d, touch_num : %d, button_check:%d, buf[1] : %d\n",
		...
		if (button_check == 0) {
			...
			for (i = 0; i < 2 && touch_num > 0; i++) {
				if (touch_id[i] >= 1) {
					g_Mtouch_info[i].status = 1;
				} else if (touch_id[i] == 0
					   && g_Mtouch_info[i].status == 1) {
				...
				input_report_abs(ts->input_dev,
						 ABS_MT_POSITION_X, posX[i]);
				input_report_abs(ts->input_dev,
						 ABS_MT_POSITION_Y, posY[i]);
				input_report_abs(ts->input_dev,
						 ABS_MT_TOUCH_MAJOR,
						 touch_id[i]);
				input_report_abs(ts->input_dev,
			...
			touchkey_status[key].key_press =
			    (button_status ? PRESS_KEY : RELEASE_KEY);
...
#ifdef __TOUCH_DEBUG__
			printk(KERN_ERR
			       "[TSP] silabs_ts_work_func: buf[%d] : %d, button_status: %d\n",
			       key, touchkey_status[key].key_value,
			       touchkey_status[key].key_press);
#endif
		}
...
static void check_ic_work_func(struct work_struct *work_timer)
{
	...
	struct silabs_ts_data *ts =
	    container_of(work_timer, struct silabs_ts_data, work_timer);
	...
	if (testmode == 1) {
		...
		if (touch_check == 0) {
			if (ret < 0) {
				tsp_reset();
				printk(KERN_ERR
				       "silabs_ts_work_func : i2c_master_send [%d]\n",
				       ret);
			}
...
static int silabs_ts_probe(struct i2c_client *client,
			   const struct i2c_device_id *id)
{
	...
	tsp_irq = gpio_to_irq(client->irq);
	...
	if (ts->input_dev == NULL) {
		...
		touch_present = 0;
		printk(KERN_ERR
		       "silabs_ts_probe: Failed to allocate input device\n");
	...
	ts->input_dev->evbit[0] =
	    BIT_MASK(EV_SYN) | BIT_MASK(EV_ABS) | BIT_MASK(EV_KEY);
	...
	if (ret) {
		touch_present = 0;
		printk(KERN_ERR
		       "silabs_ts_probe: Unable to register %s input device\n",
		       ts->input_dev->name);
	...
	gpio_request(GPIO_TP_INT, "ts_irq");
	gpio_direction_input(GPIO_TP_INT);

	if (tsp_irq) {
		ret =
		    request_threaded_irq(tsp_irq, silabs_ts_irq_handler,
					 silabs_ts_work_func,
	...
	printk(KERN_ERR "XXXXXX : i2c_master_send [%d]\n", ret);
	if (tsp_chheck == 0) {

		ret = i2c_master_recv(ts->client, &buf_firmware, 3);
		if (ret < 0) {
			printk(KERN_ERR
			       "silabs_ts_work_func : i2c_master_recv [%d]\n",
			       ret);
			tsp_chheck = 1;
		}
		printk("[TSP] silabs_ts_probe %d, %d, %d\n", buf_firmware[0],
		       buf_firmware[1], buf_firmware[2]);
...
static int silabs_ts_resume(struct i2c_client *client)
{
	...
	if (touch_present) {
		...
		/* FIXME: configure something global for init? */
#if 0
...
static int __devinit silabs_ts_init(void)
{
	...
	touch_regulator = regulator_get(NULL, REGU_NAME_TP);
	touch_regulator1 = regulator_get(NULL, REGU_NAME_TP1);
...
static void __exit silabs_ts_exit(void)
{
	if (touch_regulator) {
		...
		regulator_put(touch_regulator1);
		touch_regulator = NULL;
		touch_regulator1 = NULL;
	}
...
int silabs_quicksense(int address, int size, char *buff)
{
	...
	ret =
	    i2c_master_recv(ts_global->client, buff,
			    size + QUICKSENSE_OVERHEAD);
					...
					char *buf)
{
	int Tx_Channel = NUM_TX_CHANNEL;
	int Rx_Channel = NUM_RX_CHANNEL;
	uint8_t buffer1[NUM_TX_CHANNEL * NUM_RX_CHANNEL * 2 +
			QUICKSENSE_OVERHEAD] = { 0, };
	uint8_t buffer2[NUM_TX_CHANNEL * NUM_RX_CHANNEL * 2 +
			QUICKSENSE_OVERHEAD] = { 0, };
	uint16_t rawdata[NUM_TX_CHANNEL][NUM_RX_CHANNEL] = { {0,}, };

	uint16_t RAWDATA_MAX[108] =
	    { 14350, 14510, 14560, 14610, 14710, 14680, 14780, 14780, 14440,
13720, 13990, 14080, 14150, 14190, 14190, 14260, 14250, 13910, 13650, 13940, 14040,
14110, 14150, 14150, 14210, 14200, 13870, 13610, 13890, 14000, 14080, 14120, 14120,
14200, 14170, 13840, 13570, 13870, 13960, 14040, 14080, 14080, 14160, 14150, 13820,
13560, 13830, 13920, 13990, 14020, 14020, 14110, 14120, 13790, 13540, 13800, 13900,
13960, 14000, 14000, 14090, 14100, 13770, 13400, 13760, 13900, 13980, 14030, 14040,
14150, 14200, 14270, 13380, 13750, 13890, 13980, 14020, 14040, 14140, 14160, 14050,
13370, 13730, 13870, 13960, 14010, 14010, 14100, 14130, 13980, 13440, 13720, 13860,
13960, 14000, 14000, 14100, 14130, 14180, 14150, 14560, 14730, 14830, 14870, 14870,
14960, 15000, 15010 };
	uint16_t RAWDATA_MIN[108] =
	    { 10610, 10720, 10770, 10800, 10870, 10850, 10930, 10920, 10680,
10140, 10340, 10410, 10460, 10490, 10480, 10540, 10530, 10280, 10090, 10300, 10370,
10430, 10460, 10460, 10500, 10500, 10250, 10060, 10270, 10340, 10410, 10440, 10440,
10490, 10470, 10230, 10030, 10250, 10320, 10380, 10400, 10410, 10460, 10460, 10210,
10020, 10220, 10290, 10340, 10360, 10360, 10430, 10440, 10190, 10010, 10200, 10270,
10320, 10350, 10350, 10410, 10420, 10180, 9900, 10170, 10270, 10330, 10370, 10380,
10460, 10500, 10540, 9890, 10160, 10270, 10330, 10360, 10370, 10450, 10470, 10390,
9880, 10150, 10250, 10320, 10350, 10360, 10420, 10440, 10330, 9930, 10140, 10250,
10310, 10350, 10350, 10420, 10450, 10480, 10460, 10760, 10880, 10960, 10990, 10990,
11060, 11090, 11100 };
	...
	for (i = 0; i < 1; i++)
	{
		for (j = 0; j < Rx_Channel; j++) {
			...
			rawdata[i][j] =
			    (buffer2
			     [(i * Rx_Channel + j) * 2 + QUICKSENSE_OVERHEAD -
			      1] << 8) + buffer2[(i * Rx_Channel + j) * 2 +
						 QUICKSENSE_OVERHEAD];
			printk
			    ("[TSP] rawdata_pass_fail_silabs rawdata[%d][%d] : %d, \n",
			     i, j, rawdata[i][j]);
	...
	printk("[TSP] ver tsp=%x, HW=%x, SW=%x\n", buf_firmware_show[1],
	       buf_firmware_show[2], buf_firmware_show[0]);
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	//
	ret =
	    silabs_quicksense(BASELINE_ADDRESS,
			      NUM_TX_CHANNEL * NUM_RX_CHANNEL * 2, buffer1);
	if (ret != 0) {
		printk("[TSP] silabs_quicksense fail! %s : %d, \n", __func__,
		       __LINE__);
		return -1;
	...
	//
	ret =
	    silabs_quicksense(RAWDATA_ADDRESS,
			      NUM_TX_CHANNEL * NUM_RX_CHANNEL * 2, buffer2);
	if (ret != 0) {
		printk("[TSP] silabs_quicksense fail! %s : %d, \n", __func__,
		       __LINE__);
		return -1;
			    ...
			    (buffer2
			     [(i * Rx_Channel + j) * 2 + QUICKSENSE_OVERHEAD -
			      1] << 8) + buffer2[(i * Rx_Channel + j) * 2 +
						 QUICKSENSE_OVERHEAD];
			written_bytes +=
			    sprintf(buf + written_bytes, "%d %d\n",
				    rawdata[i][j],
				    baseline[i][j] - rawdata[i][j]);
		}
	}
				    ...
				    struct device_attribute *attr, char *buf1)
{
	...
	int Rx_Channel = NUM_RX_CHANNEL;
	int written_bytes = 0;	/* & error check */
	uint8_t buffer[NUM_TX_CHANNEL * NUM_RX_CHANNEL * 2 +
		       QUICKSENSE_OVERHEAD] = { 0, };
	uint16_t baseline[NUM_TX_CHANNEL][NUM_RX_CHANNEL] = { {0,}, };
	uint16_t button_baseline[NUM_MTRBUTTONS] = { 0, };
	int i, j, ret;
	...
	ret =
	    silabs_quicksense(BASELINE_ADDRESS,
			      NUM_TX_CHANNEL * NUM_RX_CHANNEL * 2, buffer);
	if (ret != 0) {
		printk("[TSP] silabs_quicksense fail! %s : %d, \n", __func__,
		       __LINE__);
		return -1;
	...
	for (i = 0; i < Tx_Channel; i++) {
		for (j = 0; j < Rx_Channel; j++) {
			baseline[i][j] =
			    (buffer
			     [(i * Rx_Channel + j) * 2 + QUICKSENSE_OVERHEAD -
			      1] << 8) + buffer[(i * Rx_Channel + j) * 2 +
						QUICKSENSE_OVERHEAD];
			printk(" %5d", baseline[i][j]);

			written_bytes +=
			    sprintf(buf1 + written_bytes, "%d\n",
				    baseline[i][j]);
		}
		printk("\n");
	}
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	printk("[TSP] ver tsp=%x, HW=%x, SW=%x\n", buf_firmware_ver[1],
	       buf_firmware_ver[2], buf_firmware_ver[0]);
