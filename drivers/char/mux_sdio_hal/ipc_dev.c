...
static ssize_t mcom_read(struct file *filp, char __user *buf, size_t size,loff_t *ppos)
{
    ...
    len = sprintf(buf_ptr, "\r\n\t\tspeed\trate\tmux\t sdio\t saved\tcnt\tdcnt\tovfl\tcerr\tperr\tstatus\r\n");
    ...
    {     
        speed = (ipc_ptr->sdio_count - s_write_sdio_count)*1000/jiffies_to_msecs(jiffies - s_prv_read_time);
    }
 
     len = sprintf(&buf_ptr[ret], "MCOM Write:\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%s\r\n",  \
            speed,ipc_ptr->rate, ipc_ptr->mux_count,ipc_ptr->sdio_count, ipc_ptr->saved_count, 
            ipc_ptr->connect_timeout_count,ipc_ptr->disconnect_timeout_count, ipc_ptr->overflow_count,  ipc_ptr->packet_error_count, \
    ...
    {
        speed = (ipc_ptr->sdio_count - s_read_sdio_count)*1000/jiffies_to_msecs(jiffies - s_prv_read_time);
    }

     len = sprintf(&buf_ptr[ret], "MCOM Read:\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%u\t%s\r\n",
            speed,ipc_ptr->rate,ipc_ptr->mux_count,ipc_ptr->sdio_count, ipc_ptr->saved_count, 
            ipc_ptr->connect_timeout_count, ipc_ptr->disconnect_timeout_count, ipc_ptr->overflow_count,  ipc_ptr->packet_error_count, 
            ipc_ptr->crc_error_count, _get_status_string(ipc_ptr->status)); 
